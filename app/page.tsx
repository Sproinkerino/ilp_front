...
export default function ChatInterface() {
  ...
  // Function to focus input with retry
  const focusInput = () => {
    ...
    // If immediate focus failed, try with a delay
    setTimeout(() => {
      console.log('Retry focus with delay')
      if (inputRef.current) {
        inputRef.current.focus()
      }
    }, 100)
  ...
  // Add effect to refocus when loading state changes
  useEffect(() => {
    if (!isLoading) {
      console.log('Loading finished, attempting to focus')
      focusInput()
    }
  }, [isLoading])
  ...
  const handleSendMessage = async (message: string = inputMessage) => {
    ...
    try {
      ...
      setIsLoading(false)
      // Focus will be handled by the isLoading useEffect
    }
  }
      ...
      <main className="flex-1 flex flex-col p-4 md:p-8 max-w-4xl mx-auto w-full">
        ...
        <div className="relative">
          <Input
            ...
            autoFocus
            onBlur={() => {
              console.log('Input lost focus, attempting to refocus')
              focusInput()
            }}
          />
